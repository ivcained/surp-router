...
(() => {
  const tools = [
    {name:'surp_list_models', title:'List Surp models', description:'List models and routing presets available through Surp.', inputSchema:{type:'object',properties:{},additionalProperties:false}, annotations:{readOnlyHint:true}, execute:async(_args,{signal}={})=> (await fetch('/v1/models',{signal})).json()},
    {name:'surp_get_status', title:'Get Surp status', description:'Read current Surp gateway and upstream service status.', inputSchema:{type:'object',properties:{},additionalProperties:false}, annotations:{readOnlyHint:true}, execute:async(_args,{signal}={})=> (await fetch('/api/health',{signal})).json()},
    {name:'surp_compare_prices', title:'Compare Surp prices', description:'Compare live Surp routing prices using an optional text filter.', inputSchema:{type:'object',properties:{query:{type:'string',description:'Optional model or route search text'}},additionalProperties:false}, annotations:{readOnlyHint:true}, execute:async({query=''},{signal}={})=> (await fetch('/api/price-compare?search='+encodeURIComponent(query),{signal})).json()}
  ];
